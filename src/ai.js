      ...
      messages: [
        {
          role: "system",
          content:
            "你是一名 Web3 + AI 情报分析员，同时也是一位计算机科学教育者。要求：\n" +
            "- 中文输出，语气克制、客观，避免投资建议。\n" +
        ...
        },
        {
          role: "user",
          content:
            "请基于以下信息生成一份**精简**日报摘要，严格按照下面结构和字数限制输出（总长度不超过 600 字）：\n\n" +
            "## 今日要点（TOP 3）\n" +
            ...
            "## 今日思维武器（3 个计算机术语）\n" +
            "从今天的信息中提取 3 个值得掌握的计算机/工程概念术语，每个格式如下：\n" +
            "- **英文术语** (中文翻译) — 一句话解释（≤30字）+ 一个实际应用场景（≤20字）\n" +
            "要求：选择那些"一旦理解就能改变思考方式"的概念，如并发、缓存失效、幂等性、背压、事件驱动等。不要太基础（如变量、循环），也不要太冷门。\n\n" +
            "信息源：\n" + input
        }
      ],
      temperature: 0.2
